...
Contexto importante: o upstream declara suporte só a macOS | Linux e não há nenhuma referência a Windows/MSYS nos ~116 scripts bash do repo. Mesmo assim ele funciona sob Git Bash, desde que uma condição não-óbvia seja satisfeita (item 3 abaixo). Já validei esse caminho em outra máquina — siga os passos e valide cada um em vez de assumir.
...
## 3. O bloqueador crítico — symlinks
...
## 4. Validar (não pule — instalar não é o mesmo que funcionar)
